...
# Load processor and base Wav2Vec2 model
...
if not os.path.exists(DATASETS_PATH):
    raise ValueError(f"❌ Dataset directory '{DATASETS_PATH}' not found!")
...
if len(file_paths) == 0:
    raise ValueError("❌ No audio files found matching the target emotions!")
...
class FearClassifier(nn.Module):
    def __init__(self, base_model, num_classes):
        ...
        self.pooling = AttentionPooling(self.base.config.hidden_size)
...
for layer in model.base.encoder.layers[-6:]:
    for param in layer.parameters():
        param.requires_grad = True

# Loss (with class weights to fix imbalance)
...
optimizer = torch.optim.AdamW([
    {"params": model.base.encoder.layers[-6:].parameters(), "lr": 1e-5},
...
torch.save({
    ...
    'label_encoder': le
}, "models/fear_classifier_final.pth")
print("✅ Model saved as 'models/fear_classifier_final.pth'")
